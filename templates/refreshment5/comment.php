...
<div class="comments_tff" id="comments_form">
    <noscript>
    Die Kommentarfunktion setzt Javascript voraus. Grund: viele Bots, gerade die blöden, billigen, führen kein Javascript aus. Das bremst Spam ein wenig.
    </noscript>
    ...
    <?php foreach ($this->view['comments'] as $id => $row) : ?>
        ...
        <div class="row">
            ...
            <div class="one-third column">
                <?php if ($row['url']): ?>
                    <a href="<?= $row['url'] ?>" target="_blank" rel="nofollow"><?= $row['url'] ?></a>
                <?php else: ?>
                    keine Homepage
                <?php endif; ?>
        ...
        <?php if ($this->view['USER']->get_level() == 1): ?>
                    <p>
                        <a href="<?= $this->view['PAGEROOT'] ?>comment/delete/<?= $row['cid'] ?>">Löschen</a>
                    </p>
        <?php endif; ?>
...
<?php else: ?>
    <p>Kommentare sind hier verboten. Hintergrund: Manche Leute sind zu dämlich, sich im Netz zu benehmen.</p>
<?php endif; ?>