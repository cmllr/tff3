...
//        $this->garbage_collection();
    ...
    public function doImg($params = 0)
    {
        ...
        $cached_result = $this->fetch_image($images, $widths, $heights);
        if (!$cached_result) {
            $extension = substr($images, -3, $images);
            switch ($extension) {
                case 'gif':
            ...
            $thumb = $extension != 'gif' ? imagecreatetruecolor($thumb_width,
                    $thumb_height) : imagecreate($thumb_width, $thumb_height);
            // Resize and crop
            ...
            switch ($extension) {
                case 'gif':
                    ...
                    break;
                case 'png':
                    imagepng($thumb, $this->file, 100);
                    break;
                default:
                    imagejpeg($thumb, $this->file, 100);
                    break;
            }
        }
